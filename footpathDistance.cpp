...
// Get distances from one source to multiple destinations
std::vector<double> getDistancesFromSource(const std::vector<Stop> &stops,
  ...
  std::cout << "Getting distances from stop " << sourceIndex << " ("
  ...
  if (curl) {
    ...
    if (res == CURLE_OK) {
      try {
        ...
        if (response.contains("distances") && !response["distances"].empty()) {
          std::vector<double> distances;
          for (const auto &dist : response["distances"][0]) {
            distances.push_back(dist.is_null() ? -1.0 : dist.get<double>());
          }
          return distances;
        }
      } catch (const std::exception &e) {
...
int main() {
  ...
  std::string baseFilename = "foot_distances";
  ...
  outFile << "stop_id,stop_id,distance" << std::endl;
  ...
  for (int sourceIndex = 0; sourceIndex < totalStops; sourceIndex++) {
    ...
    for (int destStart = sourceIndex + 1; destStart < totalStops;
      ...
      std::vector<double> distances =
          getDistancesFromSource(allStops, sourceIndex, destStart, destEnd);
      ...
      if (!distances.empty()) {
        ...
        for (int i = 0; i < distances.size(); i++) {
          ...
          if (distances[i] >= 0) {
            outFile << allStops[sourceIndex].stop_id << ","
                    << allStops[destIndex].stop_id << "," << distances[i]
                    << std::endl;
          } else {
            outFile << allStops[sourceIndex].stop_id << ","
        ...
        std::cerr << "Failed to get distances for source " << sourceIndex
                  << std::endl;
        failedRequests++;
      }
