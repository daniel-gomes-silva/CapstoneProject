...
int main() {
  ...
  // Maximum destinations per request (100 locations total)
  const int MAX_DESTINATIONS = 99;
  ...
  for (int sourceIndex = 0; sourceIndex < totalStops; sourceIndex++) {
    ...
    for (int destStart = sourceIndex + 1; destStart < totalStops;
      ...
      requests.push_back({sourceIndex, destStart, destEnd});
    }
  }
  std::cout << "Created " << requests.size() << " requests to be processed."
            << std::endl;

  #pragma omp parallel for schedule(dynamic)
  for (int i = 0; i < requests.size(); ++i) {
    const auto &current_request = requests[i];

    std::vector<double> durations = getDurationsFromSource(
        allStops, current_request.sourceIndex, current_request.destStart,
        current_request.destEnd);
    ...
    #pragma omp critical
    {
      if (!durations.empty()) {
        ...
        for (size_t j = 0; j < durations.size(); ++j) {
          int destIndex = current_request.destStart + j;
          totalStopPairs++;
          if (durations[j] >= 0) {
            outFile << allStops[current_request.sourceIndex].stop_id << ","
                    << allStops[destIndex].stop_id << "," << durations[j]
                    << std::endl;
          } else {
            outFile << allStops[current_request.sourceIndex].stop_id << ","
                    << allStops[destIndex].stop_id << ",-1" << std::endl;
            std::cerr << "No route found between "
                      << allStops[current_request.sourceIndex].stop_id
                      << " and " << allStops[destIndex].stop_id << std::endl;
          }
        }
        totalRequests++;
        std::cout << "Thread " << omp_get_thread_num()
                  << " completed a request. Total: " << totalRequests
                  << std::endl;
      } else {
        std::cerr << "Failed to get durations for source "
                  << current_request.sourceIndex << std::endl;
        failedRequests++;
      }
    }
  }
